...
int dice;
std::string eq = "Este e um minijogo onde serão geradas expressões para\no usuário responder.\nO programa \"para\" se o usuário errar.\nPressione \"Enter\" para começar:";
std::string in_user = "";
...
// SFML
sf::RenderWindow janela(sf::VideoMode(640, 360), "Carrossel");
sf::Font clearSans;
sf::Text equacao;
sf::Text erro_tela;
sf::Text resposta;
...
class jogo {
public:
    void fazer_e() { // faz a expressao e manda para a tela
        switch (dice) {
        case 0: // conjuntos
            a = rand() % 5;
            b = rand() % 3;
    ...
    void valida_e() { // checa e ve se a resposta esta correta
        erro_tela.setString("");
        if (in_user == "") erro_tela.setString("Nada foi digitado. Tente novamente.");
        else {
            switch (dice) {
            case 0: // conjuntos
                if (in_user != "0" && in_user != "1") { // colocou algo diferente
                    erro_tela.setString("Você colocou um(s) dígito(s) diferente(s). Tente novamente");
                    std::cout << "Voce colocou um digito diferente";
                }
                else if (((a == 0 || a == 1 && b == 1 || a > 0 && b == 2) || conjuntos[a] == casos[b].back()) ^ stoi(in_user)) { // errou
                ...
                break;
            case 1: // expressoes
                if (stoi(in_user) != te_interp(eq.data(), 0)) {
    ...
    while (janela.isOpen())
    {
        ...
        while (janela.pollEvent(event)) { // introducao
            ...
            case sf::Event::TextEntered:
                ...
                case '\b':
                    if (in_user.length() > 0) in_user.pop_back();
                    break;
                default:
                    if ((event.text.unicode == 45 || event.text.unicode > 47 && event.text.unicode < 58) && in_user.length() < 10) {
                        in_user += event.text.unicode;
                    }
                }
            }
            resposta.setString(in_user);
